...
function accion_agenda(accion){
    if (dijit.byId("frm_agenda").validate()) {
        ...
        if (devuelve_fecha(fecha_ingresar)< devuelve_fecha(fecha_inicio) ||devuelve_fecha(fecha_ingresar)> devuelve_fecha(fecha_fin) ){
            ...
            dojo.xhrPost({
                url: '/websae/administracion/F_ae_administrar_objetos?tipo='+accion+'_agenda&id_subevento='+id_subevento,
                form: 'frm_agenda',
                handleAs: 'json',
                load: function(xhr) {
                    if (xhr.tipo == "OK") {
    ...
    YAHOO.util.Event.onDOMReady(function() {
        YAHOO.example.ClientPagination = function() {
            ...
            var oConfigs = {
                ...
                initialRequest: "tipo=mostrar_agenda_subevento&id_subevento="+id_subevento
            };
...
function popup_modificar_horario(id_actividad, fecha_inicio, fecha_fin, actividad){
    dijit.byId('div_horario').show();
...
function mostrar_tabla_horario() {
    YAHOO.util.Event.onDOMReady(function() {
        YAHOO.example.ClientPagination = function() {
            var formatUrl = function(elCell, oRecord, oColumn, sData) {
                elCell.innerHTML = "&nbsp&nbsp<a href='#' onclick=\"popup_modificar_horario('" + oRecord.getData("id_actividad") + "','" + oRecord.getData("hora_inicio") + "','" + oRecord.getData("hora_fin") + "','" + oRecord.getData("actividad") + "');\"><img alt=\"ver\" width=\"25px\" src=\"/websae/images/accessories-text-editor.png\" title=\"" + parseIdioma("modificar") + "\" /></a>";
            };
            var myColumnDefs = [
            ...
            myDataSource.responseSchema = {
                resultsList: "items",
                fields: ["id_actividad","hora_inicio","hora_fin","actividad"]
            };
