...
function accion_material(accion){
    if (dijit.byId("frm_material").validate()) {
        dojo.xhrPost({
            url: '/websae/administracion/F_ae_administrar_objetos?tipo='+accion+'_material_subevento&id_subevento='+id_subevento,
            form: 'frm_material',
            handleAs: 'json',
            load: function(xhr) {
                if (xhr.tipo == "OK") {
...
function tbl_materiales() {
    var formatUrl = function(elCell, oRecord, oColumn, sData) {
        elCell.innerHTML = format_innerHTML("popup_modificar_material('" + oRecord.getData("id_material") + "','" + oRecord.getData("tipo_material.nombre") + "','" + oRecord.getData("descripcion") + "','" + oRecord.getData("precio") + "','" + oRecord.getData("cantidad_entregar") + "');","/websae/images/accessories-text-editor.png");
    },
    fields = ["id_material","tipo_material.nombre","descripcion","cantidad_entregar","precio"],
    myColumnDefs = [
        { label:parseIdioma("Nombre"), key:"tipo_material.nombre", width:130},
        { label:parseIdioma("Cantidad"), key:"cantidad_entregar"},
        { label:parseIdioma("Precio"), formatter:"currency", key: "precio"},
        { label:parseIdioma("Accion"), formatter:formatUrl }
    ]

    tbl_generica("/websae/administracion/F_ae_mostrar_datos?tipo=materiales_subevento&id_subevento="+id_subevento,fields,myColumnDefs,"tbl_materiales");
}
