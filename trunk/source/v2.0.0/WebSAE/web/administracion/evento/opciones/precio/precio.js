...
function accion_precio(accion){
    if (dijit.byId("frm_precio").validate()) {
        if (devuelve_fecha(dojo.byId('txt_fecha_inicio').value)>devuelve_fecha(dojo.byId('txt_fecha_fin').value) && accion!="eliminar" ){
            ...
            dojo.xhrPost({
                url: '/websae/administracion/F_ae_administrar_objetos?tipo='+accion+'_categoria_evento',
                form: 'frm_precio',
                handleAs: 'json',
                load: function(xhr) {
                    if (xhr.tipo == "OK") {
...
function tbl_precios() {
    ...
    tbl_generica("/websae/administracion/F_ae_mostrar_datos?tipo=mostrar_categorias_evento",fields,myColumnDefs,"tbl_precios");
}


function ventana_descuentos(id_categoria){
    window.location.href = "/websae/administracion/evento/opciones/precio/descuento/index.jsp?id_categoria="+id_categoria;
}

function tbl_descuentos() {
    var formatUrl = function(elCell, oRecord, oColumn, sData) {
        elCell.innerHTML = format_innerHTML("ventana_descuentos('" + oRecord.getData("id_categoria") + "');","/websae/images/descuento.gif");
    },
    ...
    tbl_generica("/websae/administracion/F_ae_mostrar_datos?tipo=mostrar_categorias_por_evento",fields,myColumnDefs,"tbl_descuentos");
